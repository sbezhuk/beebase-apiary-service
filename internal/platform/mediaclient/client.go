...
// Attach implements application/apiary.MediaClient by calling
...
func (c *Client) Attach(ctx context.Context, accessToken string, apiaryID, mediaID uuid.UUID) error {
	u := fmt.Sprintf("%s/api/v1/media/%s/attach", c.baseURL, mediaID)

	body, err := json.Marshal(struct {
		OwnerType string `json:"owner_type"`
		OwnerID   string `json:"owner_id"`
	}{OwnerType: "APIARY", OwnerID: apiaryID.String()})
	if err != nil {
