...
// VerifyOwnership implements application/apiary.MediaClient by calling
...
func (c *Client) VerifyOwnership(ctx context.Context, accessToken string, ids []uuid.UUID) error {
	...
	if len(body.Items) != len(ids) {
		...
		// by the same non-leaking convention apiary.ErrNotFound follows.
...
// DeleteByIDs implements application/apiary.MediaClient by calling
// media-service's DELETE /api/v1/media?ids= - used when the apiary itself
