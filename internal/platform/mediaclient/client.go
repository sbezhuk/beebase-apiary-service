// Package mediaclient implements application/apiary.MediaClient against
// the real media-service over HTTP.
package mediaclient
...
// Client implements application/apiary.MediaClient against the real
...
// already verified owns the apiary.
type Client struct {
	baseURL string
	http    *http.Client
}
...
// DeleteByOwner implements application/apiary.MediaClient.
func (c *Client) DeleteByOwner(ctx context.Context, accessToken string, apiaryID uuid.UUID) error {
	u := fmt.Sprintf("%s/api/v1/media?owner_type=APIARY&owner_id=%s", c.baseURL, url.QueryEscape(apiaryID.String()))
...
// ListAttached implements application/apiary.MediaClient.
func (c *Client) ListAttached(ctx context.Context, accessToken string, apiaryID uuid.UUID) ([]uuid.UUID, error) {
	ids := []uuid.UUID{}

	for page := 1; ; page++ {
		u := fmt.Sprintf("%s/api/v1/media?owner_type=APIARY&owner_id=%s&page=%d&limit=%d",
			c.baseURL, url.QueryEscape(apiaryID.String()), page, listPageLimit)
...
// VerifyAttached implements application/apiary.MediaClient.
func (c *Client) VerifyAttached(ctx context.Context, accessToken string, apiaryID, mediaID uuid.UUID) error {
...
// Detach implements application/apiary.MediaClient.
