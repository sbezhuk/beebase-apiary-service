...
func TestClient_ListAttached_WalksEveryPage(t *testing.T) {
	...
	client := mediaclient.New(srv.URL)
	got, err := client.ListAttached(context.Background(), "good-token", apiaryID)
	if err != nil {
...
func TestClient_Attach_Success(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["owner_type"] != "APIARY" {
			t.Errorf("owner_type = %q, want APIARY", body["owner_type"])
		}
		if body["owner_id"] != apiaryID.String() {
			t.Errorf("owner_id = %q, want %s", body["owner_id"], apiaryID)
		}

		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{
			"id": mediaID, "owner_type": "APIARY", "owner_id": apiaryID,
		})
	}))
	defer srv.Close()

	client := mediaclient.New(srv.URL)
	if err := client.Attach(context.Background(), "good-token", apiaryID, mediaID); err != nil {
